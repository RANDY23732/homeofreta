...
const ADMIN_EMAIL = 'homeofreta@gmail.com';
const CODE_TTL_SECONDS = 900; // 15 minutes
const RESEND_COOLDOWN_SECONDS = 60;

$dataDir = __DIR__ . '/data';
$storeFile = $dataDir . '/email_verifications.json';
...
function is_gmail_address($email) {
    ...
    $blocked = ['customer', 'admin', 'test', 'info', 'support', 'sales', 'contact', 'example', 'mail', 'noreply', 'no-reply', 'service', 'team', 'username', 'guest'];
    if (in_array($local, $blocked, true) || strlen($local) < 3) {
        return false;
    }
...
function gmail_mx_ok() {
    return checkdnsrr('gmail.com', 'MX');
}
...
if ($action === 'send') {
    if (!is_gmail_address($email)) {
        respond(['success' => false, 'message' => 'Please use a valid @gmail.com address that can receive mail.'], 400);
    }
    if (!gmail_mx_ok()) {
        respond(['success' => false, 'message' => 'Gmail mail servers are unreachable. Check your connection and try again.'], 503);
    }
    ...
    $subject = 'HOME OF RETA — Verify your Gmail address';
    ...
    if (!$mailResult['ok']) {
        respond(['success' => false, 'message' => $mailResult['error'] ?? 'Failed to send verification email.'], 500);
    }
    ...
    $response = [
        'success' => true,
        'message' => 'Verification code sent to your Gmail inbox. Check spam if you do not see it.',
        'expires_in' => CODE_TTL_SECONDS
    ];
    if (($mailResult['method'] ?? '') === 'local_dev') {
        error_log("HOME OF RETA verification for {$email}: {$code}");
...
if ($action === 'verify') {
    ...
    $store = load_store($storeFile);
    if (!isset($store[$email])) {
        respond(['success' => false, 'message' => 'No verification code found. Request a new code.'], 404);
    }
    ...
    if (time() > (int)($entry['expires_at'] ?? 0)) {
        unset($store[$email]);
        save_store($storeFile, $store);
        respond(['success' => false, 'message' => 'Code expired. Request a new verification code.'], 410);
    }
    ...
    unset($store[$email]);
    save_store($storeFile, $store);
    respond(['success' => true, 'message' => 'Email verified successfully.', 'verified' => true]);
}
